...
var nba = require("nba");
var bballref = require("basketball-reference");
var ruser = "";
var remind = false;
var rreminder = "";
var rmonth = "";
		...
		if((message.body.toUpperCase() === 'NBA'.toUpperCase())  && !Boolean(remind)) {
			...
			/*nba.stats.scoreboard({gameDate: gameD}, function (err, response) {
				...
				if(response){
					...
					api.sendMessage("Whoops, something went wrong", message.threadID);
				}
			...
			bballref.getLeagueStandings(yyyy, function(data) {
  ...
  for (i = 0; i < estand.length; i++) { 
    ...
    api.sendMessage("\nWestern Conference:\n" + wgames + "\n\nEastern Conference:\n" + egames , message.threadID);
		...
		if(((message.body).toUpperCase().indexOf("cake".toUpperCase())>-1) && !Boolean(remind)) {
			api.sendMessage("The cake is a lie.", message.threadID);
        }
